...
def sample_job_response():
    return {
        ...
        "context": {
            ...
            "content": [
                {"path": "/tmp/test/storage_001/file1.xlsx", "name": "file1.xlsx"},
                {"path": "/tmp/test/storage_001/file2.xlsx", "name": "file2.xlsx"},
            ],
        },
    }
...
class TestCreateReport:
    ...
    def test_create_report_validates_params(self, mock_trigger, client):
        ...
        response = client.post("/reportingJob/report_123/create", json=invalid_params)

        assert response.status_code == 422
        mock_trigger.assert_not_called()

    @patch("cdq.infra.airflow.trigger_reporting_job")
    def test_create_report_with_optional_job_id(
        self, mock_trigger, client, sample_report_params
    ):
        sample_report_params["job_id"] = "custom_job_id"
        mock_trigger.return_value = (
            201,
            {"job": {"id": "custom_job_id", "details": None}},
        )
...
class TestGetReportingJobStatus:
    ...
    def test_get_job_status_running(self, mock_fetch, client):
        ...
        running_response = {
            "status": "RUNNING",
            "context": {"workdir": "/tmp/test/storage_001", "content": []},
        }
        mock_fetch.return_value = (
            200,
            {"job": {"id": job_id, "details": running_response}},
        )
...
class TestListReportingJobs:
    ...
    def test_list_jobs_multiple(self, mock_list, client, sample_job_response):
        jobs_dict = {
            ...
            "job_2": {
                "status": "RUNNING",
                "context": {"workdir": "/tmp/test/storage_002", "content": []},
            },
            "job_3": {
                "status": "ERROR",
                "context": {"workdir": "/tmp/test/storage_003", "content": []},
            },
        }
    ...
    def test_list_jobs_with_content(self, mock_list, client):
        ...
        assert all(
            "path" in item
            for item in data["jobs"]["job_with_files"]["context"]["content"]
        )
        assert all(
            "name" in item
            for item in data["jobs"]["job_with_files"]["context"]["content"]
